...
for i_stn=1:n_stns
    ...
    for i_damp = 1:length(damp_ratio)
    %i_damp = damp_ratio(damp_num);
        for i_str = 0:n_str
            [f_vect,TF_amp_mat,TF_cpmlx_mat,lb_combs]=...
                fns_scatter_vary_damping.get_TF_scatter(n_str, n_rx, n_ry,...
                l_vect, b_vect, ftyp, V_s, L_f, B_f,damp_ratio,...
                bf_nm,i_str,cmpt,n_c,rf_fldr,cols1);

            for i_c = 1:n_c
                %% Calculating Velocity
                % using interpolation maping data from len = 501 to len = 2049
                TFcpmlx_intrp{i_damp,i_c}=interp1(f_vect,TF_cpmlx_mat{i_damp,i_c},...
                    f_inpt{i_c},'linear','extrap');
                
                % U(f) = 2*Vsensor(f)*TF(f)
                Ucmplx_mat{i_damp,i_c}=2*ff_Ucmplx_mat{i_c}.*TFcpmlx_intrp{i_damp,i_c};
                
                % V(f) = 2*pi*f*i*U(f) = i*omega*U(f)
                Vabs_mat{i_damp,i_c}=abs(Ucmplx_mat{i_damp,i_c}.*1i.*f_inpt{i_c}*2*pi);
                Vcmplx_mat{i_damp,i_c}=(Ucmplx_mat{i_damp,i_c}.*1i.*f_inpt{i_c}*2*pi);
    
                %Calculate DB
                Vss_mat{i_damp,i_c}=20*log10(Vabs_mat{i_damp,i_c}(nzero:end,:)./v_ref);
    
                %Apply filter transfer to KB(f)
                V_KB_freq{i_damp,i_c} = get_KB_freq(Vcmplx_mat{i_damp,i_c},f_inpt{i_c});
                
            end
            %Original data
            Vss_zCell{i_damp,i_str+1} = Vcmplx_mat{i_damp,3};
            Vss_xCell{i_damp,i_str+1} = Vcmplx_mat{i_damp,1};
            Vss_yCell{i_damp,i_str+1} = Vcmplx_mat{i_damp,2};
    
            %Data with filter: KB(f)
            V_KB_freq_zCell{i_damp,i_str+1} = V_KB_freq{i_damp,3};
            V_KB_freq_xCell{i_damp,i_str+1} = V_KB_freq{i_damp,1};
            V_KB_freq_yCell{i_damp,i_str+1} = V_KB_freq{i_damp,2};
        ...
        for i_flur = 1:n_str+1
            %% IFFT original data
            Vss_Zmat = Vss_zCell{i_damp,i_flur};
            Vss_Xmat = Vss_xCell{i_damp,i_flur};
            Vss_Ymat = Vss_yCell{i_damp,i_flur};
            ...
            %Vz_ifft_damp_ratio{i_damp,i_flur} = Vz_ifft;
            %Vx_ifft_damp_ratio{i_damp,i_flur} = Vx_ifft;
            %Vy_ifft_damp_ratio{i_damp,i_flur} = Vy_ifft;

            max_Vxmat{i_damp}(i_stn,:,i_flur)=max(Vx_ifft);
            max_Vymat{i_damp}(i_stn,:,i_flur)=max(Vy_ifft);
            max_Vzmat{i_damp}(i_stn,:,i_flur)=max(Vz_ifft);


            %% IFFT data with filter
            V_KB_Zmat = V_KB_freq_zCell{i_damp,i_flur};
            V_KB_Xmat = V_KB_freq_xCell{i_damp,i_flur};
            V_KB_Ymat = V_KB_freq_yCell{i_damp,i_flur};
            ...
            %Vx__KB_ifft_damp_ratio{i_damp,i_flur} = Vx_KB_ifft;
            %Vy__KB_ifft_damp_ratio{i_damp,i_flur} = Vy_KB_ifft;
            ...
            max_Vx_KB_f_mat{i_damp}(i_stn,:,i_flur)=max(KB_f_x);
            max_Vy_KB_f_mat{i_damp}(i_stn,:,i_flur)=max(KB_f_y);
            max_Vz_KB_f_mat{i_damp}(i_stn,:,i_flur)=max(KB_f_z);
...
ylbl=ylbl_vect{1}
cmp=cmpt{1};
fns_unitgeomdb.plot_DIN4150_3_XYZ_varyDamp(v_ref,n_str+1,f_x_max,max_Vxmat,V_s,n_stns,stn_vect,name_evnt,ylbl,cmp)

ylbl=ylbl_vect{2}
cmp=cmpt{2};
fns_unitgeomdb.plot_DIN4150_3_XYZ_varyDamp(v_ref,n_str+1,f_y_max,max_Vymat,V_s,n_stns,stn_vect,name_evnt,ylbl,cmp)
...
%%% Evaluation
%% For each wll i_damperation, the focus is on the top floor only, since the
%% top floor usually have the largest displacement.
%i_floor = 4; % top floor
%criteria = fn_A_value_evaluating_Kbf("ReinesWohngebiet","day");
%disp("------------------------------------------------------");
%disp("A value criteria test");
%for num_damp_ratio = 1: length(KB_f_x)
%    Vz_eval = max(KB_f_z{num_damp_ratio,i_floor});
%    Vx_eval = max(KB_f_x{num_damp_ratio,i_floor});
%    Vy_eval = max(KB_f_y{num_damp_ratio,i_floor});
%    disp("X dir");
%    A_test = fn_evaluate_A_criteria(Vx_eval,0,criteria,false);
%        disp(strcat(['  i_damp ',num2str(num_damp_ratio),':  '], A_test));
%    disp("Y dir");
