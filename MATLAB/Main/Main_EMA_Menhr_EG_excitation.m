%% Compute the FRF according to the Menhir data, assuming that the input is the singal at EG.
% Only focus on the case that all three sensor at each floor activated at the same time 
...
for i_file = 1:length(mat_tile_list)
    ...
    subplot(3,1,2);
    % In the function, Func_PSD_FRF_COH, it is used to transfer from ACC to
    % DISP, here the menhir data is VEL, therefore we have to integral it
    % once.
    res_Men.FRF = 1i*2*pi*res_Men.f.*res_Men.FRF;
