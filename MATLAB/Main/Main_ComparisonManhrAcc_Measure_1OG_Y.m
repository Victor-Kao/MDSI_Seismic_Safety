...
lowCutoffFreq = 8; % Lower cutoff frequency (40 Hz)
highCutoffFreq = 40; % Upper cutoff frequency (60 Hz)
filterOrder = 6; % Filter order
% Normalize the cutoff frequencies
...
for i_file = 1:length(mat_tile_list)
    ...
    Nperiod = 50; % The IRF is computed for 30 period on each mode
...
for i_file = 1:length(mat_tile_list)
    ...
    load(mat_tile_list{i_file});
    az = double(timeSeriesData.Data(8,:));
    % Normalize the cutoff frequencies
...
for j = 1:length(Table_SI_Menhir)
    scatter(Table_SI_Menhir{j,1},Table_SI_Menhir{j,2},"MarkerEdgeColor","#0072BD")
    hold on 
end
...
for k = 1:length(Table_SI_Acc)
    ...
    scatter(Table_SI_Acc{k,1},Table_SI_Acc{k,2},"MarkerEdgeColor","#D95319")
    hold on 
end