...
k = 1;
figure
%sgtitle('FRF', 'Interpreter', 'latex','FontSize',16)
...
for i_pos = 1:15%length(mat_tile_list)
    ...
    %plot(res_Men.f, imag(res_Men.FRF),'DisplayName', ['Channel ',num2str(i_pos)]);
    if ismember(i_pos, [9,10,11,12])
        plot(res_Men.f, imag(res_Men.FRF),'-.','DisplayName', ['Channel ',num2str(i_pos)],'LineWidth',1);
    else
        plot(res_Men.f, imag(res_Men.FRF),'--','DisplayName', ['Channel ',num2str(i_pos)],'LineWidth',1);
    end
    hold on
    k = k +1;
    title('Imag(FRF) from test 13, EX22', 'Interpreter', 'latex','FontSize',14);
    xlabel('Freq (Hz)', 'Interpreter', 'latex','FontSize',14);
    ylabel('Imag', 'Interpreter', 'latex','FontSize',14);
...
for i_pos = 1:15%length(mat_tile_list)
    ...
    %% Plot the Frequency Response Function (FRF)
    if ismember(i_pos, [9,10,11,12])
        plot(res_Men.f, real(res_Men.FRF),'-.','DisplayName', ['Channel ',num2str(i_pos)],'LineWidth',1);
    else
        plot(res_Men.f, real(res_Men.FRF),'--','DisplayName', ['Channel ',num2str(i_pos)],'LineWidth',1);
    end
    hold on
    k = k +1;
    %title('FRF from test 13, EX22', 'Interpreter', 'latex','FontSize',14);
    title('Real(FRF) from test 13, EX22', 'Interpreter', 'latex','FontSize',14);
    xlabel('Freq (Hz)', 'Interpreter', 'latex','FontSize',14);
    ylabel('Real', 'Interpreter', 'latex','FontSize',14);
    xlim([low_freq,high_freq])
    grid on
    legend("show")
end
